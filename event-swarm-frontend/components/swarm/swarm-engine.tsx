...
export function SwarmEngine({ jobs, setJobs, activeJob, setActiveJob }: any) {
  ...
  // --- UPGRADED DYNAMIC ANIMATION LOOP ---
  useEffect(() => {
    let flowInterval: NodeJS.Timeout;
    
    // Map the backend phase names to the frontend nodeRef names
    ...
    // If we have a phase, and it maps to a valid node, start animating!
    if (currentPhase && phaseToNodeMap[currentPhase]) {
      const targetNode = phaseToNodeMap[currentPhase];
      
      // Don't animate supervisor-to-supervisor, only supervisor-to-specialist
      if (targetNode !== "supervisor") {
        flowInterval = setInterval(() => {
          const id = `flow-${Date.now()}`;
          setPackets((p) => [...p, { id, from: "supervisor", to: targetNode }]);
